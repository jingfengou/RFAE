...
def show_data_figures_with_keyfeature(p_data, p_key_feature_catch, w=20, h=20, columns=20):
    ...
    for i in range(0, len(p_data)):
        ...
        for key_feature_catch_i in np.arange(len(p_key_feature_catch)):
            plt.scatter(p_key_feature_catch[key_feature_catch_i][1], p_key_feature_catch[key_feature_catch_i][0], s=0.5,
                        color='r')
    plt.tight_layout()
    plt.show()
...
def show_hierarchy_feature_selection(p_file_name, p_test_data, p_selection_hierarchy=1, p_sample_number=40,
                                     p_key_number=36):
    file = h5py.File(p_file_name, 'r')
    data = file['feature_selection']['feature_selection']['kernel:0']
...
def show_data_figures_with_hierarchy_keyfeature(p_data, p_key_feature_catch, w=20, h=20, columns=20):
    ...
    for i in range(0, len(p_data)):
        ...
        for key_feature_catch_i in np.arange(len(p_key_feature_catch_i)):
            plt.scatter(p_key_feature_catch_i[key_feature_catch_i][1], p_key_feature_catch_i[key_feature_catch_i][0],
                        s=10, color='r')
    plt.tight_layout()
    plt.show()
...
def ETree(p_train_feature, p_train_label, p_test_feature, p_test_label, p_seed):
    ...
    train_acc = accuracy_score(np.array(p_train_label), clf.predict(p_train_feature))
    ...
    test_acc = accuracy_score(np.array(p_test_label), clf.predict(p_test_feature))
