...
def cal(p_data_arr, p_label_arr_onehot, p_key_feture_number, p_epochs_number, p_batch_size_value, p_is_use_bias, p_seed,
        learning_rate, lamda1, lamda2, lamda3, dataset, modelname, device):
    if dataset=='MNIST'or'MNIST-Fasion':
        C_train_x, C_test_x, C_train_y, C_test_y = train_test_split(p_data_arr, p_label_arr_onehot, test_size=0.4,
                                                                    random_state=p_seed)
    ...
    class FractalLoss(nn.Module):
        ...
        def forward(self, x, output, target, y1, y2, WI, lamda1, lamda2, lamda3):
            ...
            loss = loss1(x, y1) + lamda1 * loss1(x, y2) + lamda2 * torch.norm(torch.exp(WI), p=1) + lamda3 * loss2(output, target)

            return loss

    class model(nn.Module):
        def __init__(self, feature_num, k, output_shape):
    ...
    if not os.path.exists(directory):
        os.makedirs(directory)
    train_dataset = MyDataset(x_train, y_train_onehot)
    # test_dataset = MyDataset(x_test, x_test)
    val_dataset = MyDataset(x_validate, y_validate_onehot)
    ...
    t_start = time.time()
    label_shape = int(np.max(y_train_onehot)) + 1

    MiceProtein_TFAE = model(feature_num, p_key_feture_number, label_shape)
    MiceProtein_TFAE = MiceProtein_TFAE.to(device=try_gpu(device))
    MiceProtein_TFAE.fit(train_loader, val_loader, p_epochs_number, learning_rate, p_key_feture_number, lamda1,
                         lamda2, lamda3)
    t_used = time.time() - t_start

    write_to_csv(np.array([t_used]),
                 "../" + dataset + "/" + modelname + "_" + str(p_key_feture_number) + "/log/" + str(
                     modelname) + str(p_key_feture_number) + "_time.csv")
    # --------------------------------------------------------------------------------------------------------------------------------
    x_test_torch = torch.tensor(x_test)
    x_test_torch = x_test_torch.to(device=try_gpu(device))
    p_data1, p_data2, output = MiceProtein_TFAE(x_test_torch, p_key_feture_number)
    ...
    print("MSE for feature selection layer",
          np.sum(np.power(np.array(p_data2.detach().cpu()) - x_test, 2)) / numbers)
    ...
    write_to_csv(results.reshape(1, len(results)),
                 "../" + dataset + "/" + modelname + "_" + str(p_key_feture_number) + "/log/" + str(
                     modelname) + str(p_key_feture_number) + "_results.csv")
    write_to_csv(selected_position_list.reshape(1, len(selected_position_list)),
                 "../" + dataset + "/" + modelname + "_" + str(p_key_feture_number) + "/log/" + str(
                     modelname) + str(p_key_feture_number) + "_selected_list.csv")
